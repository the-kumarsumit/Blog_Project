...
export const aiTitleRespone = TryCatch(async (req, res) => {
  const { text } = req.body;
...
export const aiDescriptionRespone = TryCatch(async (req, res) => {
  const { title, description } = req.body;

  const prompt =
    description === ""
      ? `Generate only one short blog description based on this title: "${title}". Your response must be only one sentence, strictly under 30 words, with no options, no greetings, and no extra text. Do not explain. Do not say 'here is'. Just return the description only.`
...
export const aiBlogResponse = TryCatch(async (req, res) => {
  ...
  return res.status(200).json({ html: cleanedHtml });
});
